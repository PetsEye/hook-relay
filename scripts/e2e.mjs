...
const RECEIVER_URL = `http://host.docker.internal:${RECEIVER_PORT}/hook`;
const DEAD_URL = `http://host.docker.internal:59999/hook`;
const ENDPOINT_ID = process.env.E2E_ENDPOINT_ID ?? "ep-e2e";
...
async function ingest(payload, idemKey) {
  ...
  if (res.status !== 202 && res.status !== 200) throw new Error(`ingest rejected: ${res.status} ${await res.text()}`);
  return res.json();
}

async function getDelivery(idemKey) {
  const list = await (await fetch(`${WEB}/api/events?q=${encodeURIComponent(idemKey)}`)).json();
  return list.events?.[0]?.deliveries?.[0];
}

async function waitFor(idemKey, predicate, label, timeoutMs = 30_000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const delivery = await getDelivery(idemKey);
    if (delivery && predicate(delivery)) return delivery;
    await sleep(400);
  }
  throw new Error(`timeout waiting for ${label} (idem ${idemKey})`);
}
...
async function main() {
  ...
  const okIdem = `e2e-ok-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const { eventId } = await ingest({ e2e: "happy", at: Date.now() }, okIdem);
  const okDelivery = await waitFor(okIdem, (d) => d.status === "success", "first success", 20_000);
  ...
  const failIdem = `e2e-fail-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const failing = await ingest({ e2e: "paperclip", at: Date.now() }, failIdem);
  const failed = await waitFor(failIdem, (d) => d.attempts >= 2 || d.status === "dlq", "at least one retry", 30_000);
  ...
  const replayed = await waitFor(failIdem, (d) => d.status === "success", "replay success", 30_000);
...
main().catch((err) => {
  console.error(`[e2e] fatal: ${err.message}`, err.cause ?? err.stack);
  process.exit(1);
});
